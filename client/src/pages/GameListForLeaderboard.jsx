...
function GameCard({ game, setSelectedGame, selectedGame }) {
    ...
    return (
        <div className={`flex flex-col p-3 w-36 h-36 bg-light-b dark:bg-dark-b rounded-lg cursor-pointer sm:w-60 sm:h-60 shadow-cyan-400/60 transition-shadow ${isSelectedGame && "shadow-lg"}`} onClick={handleGameSelect}>
            <img src={game.imageSrc} alt="game preview" className="h-[70%] object-cover rounded-t-lg" draggable="false" />
            <div className="flex flex-col items-center p-2">
                <h2 className={`text-sm sm:text-lg text-center whitespace-nowrap ${isSelectedGame && "text-primary dark:text-primaryDark"}`}>{game.name}</h2>
            </div>
        </div>
    );
}
